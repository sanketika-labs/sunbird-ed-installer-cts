...
if [ $? == 0 ]
	...
	delete "{{ .Values.global.private_container_name }}/telemetry-data" "derived/backup-$backup_key" >> "$DP_LOGS/$end_date-$1-replay.log"
else
	echo "$1 replay failed" >> "$DP_LOGS/$end_date-$1-replay.log"
 	rollback "{{ .Values.global.private_container_name }}/telemetry-data"  "derived/$backup_key" "backup-$backup_key" >> "$DP_LOGS/$end_date-$1-replay.log"
 	delete "{{ .Values.global.private_container_name }}/telemetry-data" "derived/backup-$backup_key" >> "$DP_LOGS/$end_date-$1-replay.log"
fi